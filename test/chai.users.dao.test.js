...
describe('CHAI Testing GET method of User DAO', () => {
  ...
  it('The array returned by GET must be empty', async function () {
    const result = await this.userDao.getAll();
    expect(result).to.be.deep.equal([]);
  });

  it('The array returned by the GET must have one element', async function () {
    const result = await this.userDao.getAll();
    expect(result).to.be.deep.equal([]);
  });

  it('The method must Create a User', async function () {
    const user = {
